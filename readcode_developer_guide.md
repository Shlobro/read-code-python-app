...
- `app/src/main/java/com/example/readcode/MainActivity.kt`: Current entry point and main Compose UI flow. It contains navigation state, problem data, and screen composables. If this file grows further, split data models, seed content, and screens into separate files.
...
- Problem definitions are hard-coded seed data for now. A future content system can move these into separate files or a local database.
...
- If UI or data logic expands, split `MainActivity.kt` before it becomes difficult to reason about.
